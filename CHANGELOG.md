...
## ver0.43.8 - 炮塔升级外观细节增强

- 移除普通升级等级依赖的通用外框叠圈，不再只靠两层框线表示升级。
- 基础塔升级后增加装甲肩板、能量电容、强化炮管导轨和核心细节。
- 速射塔升级后增加弹匣、供弹块、散热导轨，并在满级保留三联枪管的实际外形变化。
- 散弹塔升级后增加侧向弹仓、弹壳架、加宽炮口和满级爆破稳定翼。
- 重炮塔升级后增加重型护甲、炮管支架、反冲导轨和满级高能核心块。
- 狙击塔升级后增加稳定支架、瞄准镜组、强化长轨和满级处决棱镜。
- 增幅塔升级后增加外置中继节点和满级能量矩阵，使图鉴和游戏内预览都能直接看到更复杂的升级外形。
...
- 怪物受到正向伤害时增加短暂闪白反馈，让炮塔命中结果更清晰。
- 程序绘制怪物在原本外形上绘制半透明白色覆盖层，不额外重做怪物图案。
- 使用图片帧动画的基础怪物继续显示原有行走帧，只在其上方叠加短暂白色覆盖层并同步提亮。
- 闪白只影响视觉表现，不改变敌人血量、速度、奖励、死亡或到达终点逻辑。

验证：
- Godot 4.7 `Enemy.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。

---

## ver0.43.10 - 64px网格范围比例修复

- 将炮塔射程、炮弹溅射、精英死亡眩晕、范围卡牌半径按旧32px到当前64px网格的2倍比例换算，保持原本覆盖格数不变。
- 建造预览和卡牌拖拽范围预览同步显示缩放后的实际世界范围，大范围卡牌预览超过上限时改为稳定钳制，不再回落到默认小圈。
- 尖刺路障部署后保存的是实际世界半径，读取陷阱缓存时不再用旧32px上限重新规范化，避免大半径陷阱被压回默认值。
- 仅修正世界半径换算，不改变伤害、攻速、持续时间、金币、敌人属性或卡牌倍率逻辑。

验证：
- Godot 4.7 `Tower.gd`、`Main.gd`、`GameUI.gd`、`BuildManager.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。

---

## ver0.43.11 - 重炮炮弹射程终点爆炸

- 重炮炮弹未命中敌人时，会在飞到最大射击范围终点后自动爆炸。
- 射程终点爆炸不会对射程外目标结算直击伤害，只会以终点为中心结算溅射伤害。
- 炮弹单帧飞行距离超过剩余射程时会钳制到精确终点，避免越过射程后再命中敌人。
- 该爆炸逻辑仅作用于带重炮视觉与溅射伤害的炮弹，不改变速射、散弹、狙击等非重炮弹道行为。
...
## ver0.43.12 - 主题像素地图背景与道路图块

- 根据当前64px网格与10个关卡地图尺寸，为每个主题生成独立像素风背景图，统一保存到 `assets/maps/<theme>/background.png`。
- 为每个主题生成64x64单格道路图块：普通直路、十字交叉路、拐弯路，主题之间使用独立文件夹区分。
- 地图渲染优先读取主题背景图，不再用旧的程序色块覆盖背景。
- 路径渲染优先使用主题道路图块，并根据相邻道路自动旋转直路与拐弯路；三向和四向连接复用十字交叉图块。
- 保留道路、地面和网格缓存数据，避免破坏现有冒烟测试与后续逻辑检查。

验证：
- 已检查10个主题背景尺寸均与当前代码地图像素尺寸一致，所有道路图块均为64x64。
- Godot 4.7 `MapRenderer.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `PerformanceBenchmark.gd`，结果通过（退出码0）：avg 6.906ms，p95 6.963ms，max 9.698ms，估算 144.8 FPS。

---

## ver0.43.13 - 道路拓扑绘制修复与三向路块

- 为10个主题补齐64x64三向道路图块 `road_t.png`，三向口不再复用十字路口图块。
- 地图道路视觉连接改为从怪物实际 `spawn_paths` 连续路径计算，只绘制路线真实经过的方向。
- 保留相邻道路格用于建造阻挡和缓存，但不再用“相邻格存在”推断视觉连接，避免两条平行贴近道路被误画成十字或三岔。
- 三向、十字、直路、拐弯、端点道路现在按连接方向分别选择对应图块并旋转。
- 冒烟测试新增道路拓扑断言，覆盖大地图中“相邻但不连通”的道路格，防止回退到错误的邻格推断逻辑。

验证：
- 已检查10个主题 `road_t.png` 均为64x64。
- Godot 4.7 `MapRenderer.gd`、`LogicSmokeTest.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `PerformanceBenchmark.gd`，结果通过（退出码0）：avg 7.093ms，p95 9.480ms，max 18.209ms，估算 141.0 FPS。

---

## ver0.43.14 - 主题基地、召唤门与地图装饰物

- 为10个地图主题新增匹配背景色调的像素风 `base.png` 与 `spawn_gate.png`，基地和怪物召唤门不再使用一套固定程序色块。
- 每个主题新增两类英文命名装饰物资源：一类3帧动态装饰 `decor_01_0.png`~`decor_01_2.png`，一类1帧石头/碎块装饰 `decor_02_0.png`。
- 地图渲染会按关卡确定性随机分布装饰物，装饰物不会生成在道路格上，并使用低频逐帧切换模拟动态效果。
- 在带装饰物的格子上建造普通炮塔时，会立即摧毁该格装饰物；增益塔叠加不重复处理装饰物。
- 冒烟测试新增主题资源加载、装饰物避开道路、建塔摧毁装饰物等断言。

验证：
- 已检查10个主题的 `base.png` 均为96x96，`spawn_gate.png` 均为64x80，装饰物均为48x48。
- Godot 4.7 `MapRenderer.gd`、`WorldDecalRenderer.gd`、`BuildManager.gd`、`LogicSmokeTest.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `PerformanceBenchmark.gd`，结果通过（退出码0）：avg 6.940ms，p95 8.512ms，max 12.489ms，估算 144.1 FPS。

---

## ver0.43.15 - 程序化交互音效

- 新增程序生成的厚重建塔放置音效，不依赖外部音频素材。
- 普通炮塔建造成功时播放低频落座、木质敲击和金属锁扣混合的确认反馈。
- 建塔音效第二段锁扣回弹改为更低频的下沉回落，削弱高频金属边缘，避免听感像上扬。
- 为 UI 按钮统一绑定程序生成的短促点击音效。
- 冒烟测试新增建塔音效、按钮点击音效和按钮音效绑定断言。

验证：
- Godot 4.7 `Main.gd`、`LogicSmokeTest.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `PerformanceBenchmark.gd`，结果通过（退出码0）：avg 6.924ms，p95 8.023ms，max 13.671ms，估算 144.4 FPS。

---

## ver0.43.16 - 五类敌人移动帧接入

- 将 `generated` 中的五类敌人移动帧整理到正式资源目录 `assets/enemies/<type>/walk_01.png`~`walk_04.png`。
- `grunt`、`runner`、`brute`、`shield`、`elite` 现在都会像原 `enemy1` 一样使用对应的四帧行走动画。
- 敌人图鉴和预览复用同一套 `EnemyAnimatedSprite` 资源路径，不再用程序绘制外形替代已有素材。
- 受击闪白继续覆盖在动画帧上方，不额外生成带血条或实体 UI 的图鉴素材。

验证：
- Godot 4.7 `Enemy.gd`、`LogicSmokeTest.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。

---

## ver0.43.17 - 移除旧 enemy1 资源依赖

- 步兵已切换到 `assets/enemies/grunt/` 的新四帧移动素材，删除旧 `enemy1/` 行走帧目录。
- README 中的运行资源说明改为 `assets/enemies/` 与 `assets/maps/`。
- 代码风格断言同步到新的敌人动画帧缓存命名，避免继续引用旧 `enemy1` 缓存名。

验证：
- Godot 4.7 `Enemy.gd`、`CodeStyleAuditTest.gd`、`LogicSmokeTest.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `CodeStyleAuditTest.gd`，当前仍被既有风格审计项阻断（退出码1），阻断点分布在 `Tower.gd`、`Projectile.gd`、`Main.gd`、`WaveManager.gd` 等旧断言，不是本次 `enemy1` 资源移除引入。

---

## ver0.43.18 - 卡牌范围预览相机缩放修复

- 卡牌拖拽范围预览继续使用与实际结算一致的世界半径，但绘制到 UI 层前会按当前 Camera2D 缩放换算成屏幕半径。
- 主场景在相机缩放、相机边界钳制和常规帧更新时同步 UI 的世界到屏幕比例，避免大地图缩小时预览圈看起来比真实范围大。
- 冒烟测试新增相机缩小时卡牌预览半径断言，并覆盖第10关全地图缩放后的 UI 预览比例同步。

验证：
- Godot 4.7 `GameUI.gd`、`Main.gd`、`LogicSmokeTest.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
...
- 每次加载关卡后都会把 Camera2D 重置到当前地图可显示全图的最小缩放值，也就是进入关卡时固定为最大视野。
- 选关、下一关和控制台切关都会走同一套视角重置逻辑，避免沿用上一关或测试中手动拉近后的镜头。
- 重置视角后会重新居中并钳制到地图边界，同时同步卡牌范围预览的世界到屏幕缩放比例。
- 冒烟测试新增选关、下一关、控制台切关后的相机缩放断言。
...
## ver0.43.20 - 中下方状态提示淡出

- 将原本右上角的战斗状态提示移动到画面中间偏下位置，避免玩家视线频繁离开战场中心。
- 状态提示改为居中显示的半透明 toast 样式，带轻微暗底和文字阴影，提高地图背景上的可读性。
- 每次提示内容更新后会先短暂停留，再用平滑曲线逐渐淡出并隐藏，不再一直占据右上角。
- 冒烟测试新增提示位置、居中对齐、出现时不透明和淡出隐藏断言。

验证：
- Godot 4.7 `GameUI.gd`、`LogicSmokeTest.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。

---

## ver0.43.21 - 状态提示轻量浮字化

- 将中下方状态提示从大块 toast 面板改为更轻的浮动文字，只保留文字阴影来保证可读性。
- 缩小提示占用区域，避免遮挡战场主体和炮塔/敌人观察。
- 保留短暂停留后逐渐淡出的节奏，让提示出现得更自然但不打扰操作。
- 冒烟测试同步检查提示尺寸保持紧凑，并确认没有自定义大背景面板。
...
## ver0.43.22 - 残余代码清理与健壮性加固

- 删除 `GameUI.gd` 中已废弃的 UI 粒子弹窗管线，保留当前回弹式弹窗动画，避免旧粒子效果回流。
- 删除 `Main.gd` 中不再调用的旧道路寻路 helper，并把路径返回改成快照，避免外部修改内部缓存。
- 炮塔、投射物、敌人路径和波次敌人类型增加防御式清洗/校验，减少配置异常、节点已释放或缓存被外部持有时的崩溃风险。
- 在 64px 网格范围缩放、相机重置、地图道路连接缓存、敌人动画帧加载等容易误改位置补充中文注释。
- 更新美术说明中的敌人帧路径，避免继续指向已删除的旧 `enemy1` 目录。

验证：
- Godot 4.7 `WaveManager.gd` 脚本语法检查通过。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。

---

## ver0.44.0 - 炮塔分支、卡牌节奏与浮字提示

- 基础塔升级线调整为：等级2仍为强化核心，等级3可选择火塔或冰塔两条分支。
- 火塔分支提供持续灼烧效果，冰塔分支提供减速控制效果；全屏火焰类卡牌也接入灼烧状态。
- 图鉴支持浏览炮塔升级阶段，基础塔会展示等级1、等级2、等级3火塔和等级3冰塔四种形态，并继续复用真实 `Tower` 绘制节点。
- 卡牌掉落权重改为动态计算：基地血量低时更容易给高级治疗/防御卡，敌人密集时更容易给攻击卡，金币不足时更容易给资源卡。
- 新增战斗中每30秒自动补一张新卡的机制，满手牌时仍沿用待处理卡牌规则。
- 战术改签改为随机替换一张旧手牌，并生成另一张稀有度更高的随机卡牌，不再固定改签最左侧手牌。
- 后期经济继续收紧，降低清波金币奖励；五关后每波怪物数量增加，保持后期压力。
- 状态信息显示改为中下方堆叠上浮文字，多条提示会向上层叠并逐渐淡出。
- 修复上浮状态提示在开始页面直接显示的问题：未进入游戏时不再把开始页说明传给战斗浮字，返回开始页时会清空残留提示。
- 略微增强基础塔、速射塔、散弹塔、重炮塔和狙击塔的弹丸速度；根据后续纠正，额外只下调速射塔各级射击间隔，狙击枪后续数值不再继续扩展。
- 速射塔基础射击间隔从 `0.35` 调整为 `0.30`，等级2与等级3的间隔倍率进一步降低，让升级后的弹幕感更明显。
- 冒烟测试补充图鉴升级浏览、动态卡牌权重、30秒补卡、战术改签、上浮提示堆叠、开始页清空浮字、后期怪物数量和速射塔射击间隔断言。
...
- 修复奖励卡牌描述仍显示基础数值的问题：卡牌归一化并按稀有度缩放后，会重新生成说明文案。
- 覆盖伤害、治疗、金币、持续时间、增益倍率、减速倍率、概率、陷阱次数、灼烧和悬赏奖励等字段，避免紫卡/金卡/红卡实际效果与 UI 描述不一致。
- 补充冒烟测试，遍历20种卡牌和白、蓝、紫、金、红五档稀有度，断言描述必须包含归一化后的真实数值。

验证：
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。

## ver0.44.2 - 波次倒计时与浮字去重

- 将自动波次间隔倒计时从普通上浮提示中拆出，新增屏幕中央的大字波次倒计时面板。
- 倒计时面板显示“第 N 波即将开始”和独立倒数，避免每帧刷新小数秒时刷出一堆状态浮字。
- 上浮信息栈改为按逻辑提示事件入栈：同一次提示事件只保留一条浮字，不再因为复用 Label 时机错误被复制成两条。
- 冒烟测试补充中央倒计时显示、倒计时结束隐藏、倒计时不污染浮字提示、同一逻辑事件不重复入栈等断言。

验证：
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。

## ver0.44.3 - 倒计时显示与刷卡节奏微调

- 波次中央倒计时改为纯浮字显示，移除面板边框和装饰线。
- 波次倒计时数字改为保留小数点后两位，便于观察自动开波前的真实剩余时间。
- 自动刷卡间隔从30秒延长到60秒，并且计时从玩家点击“开始波次”成功开波后才开始。
- 冒烟测试补充波次倒计时纯文字、两位小数，以及开始波次后才启动自动刷卡计时的断言。

验证：
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。

## ver0.44.4 - 波次经济与后期压力调整

- 状态提示改为由主逻辑分配事件序号：连续两次相同文案也会按两次真实事件显示，不再被纯文本去重误挡。
- 取消清波直接金币奖励，清波只发奖励卡牌；保留 `_get_wave_clear_gold_bonus()` 兼容入口但固定返回 `0`。
- 取消敌人金币奖励随后续波次上涨的基础增量，敌人奖励只保留类型差异和当前已关闭的刷怪序号奖励入口。
- 放慢刷怪节奏，提高基础刷新间隔和最小刷新间隔，避免后期大图多路线瞬间挤满。
- 第八关之后开始显著提高每波敌人数量，后期关卡压力改为靠敌群规模而不是金币膨胀支撑。
- 冒烟测试补充连续相同提示、清波金币为零、后续波次敌人金币不涨、刷怪间隔放慢和第八关后敌群激增断言。
...
- 新增武器塔“密集火炮”：初始发射6枚跟踪弹头，每枚伤害6，射程高于重炮，弹速低于速射塔以形成重型齐射节奏。
- 设置升级梯度：等级2提升到8枚弹头、单枚伤害9；等级3提升到10枚弹头并解锁小范围爆炸，直击伤害保持等级2数值。
- 新增跟踪弹头运动逻辑：目标存活时逐帧转向目标；若目标已死亡或失效，则不重新索敌，按当前方向继续飞行。
- 为密集火炮补充64px程序绘制外观、专用弹头外观、开火/命中特效配置，并加入建造列表、图鉴配置和解锁节奏。
- 冒烟测试补充密集火炮配置、升级价格/属性、等级3范围伤害、图鉴列表可见性，以及目标死亡后弹头继续直飞断言。
...
- 新增奖励卡牌“齐射指令”：拖拽到地图释放后，所有已建武器塔会在10秒内朝释放位置持续射击。
- 炮塔新增强制射击状态，期间不依赖敌人索敌也会按自身射速开火；密集火炮在无目标齐射时改为按指令方向直飞，不强行挂接已死亡或不存在的目标。
- 支援类增幅塔不会响应齐射指令，避免无攻击塔被错误纳入武器开火逻辑。
- 卡牌 UI 增加“齐”图标、橙色战术配色、拖拽范围提示和控制台 `volley` 调试入口。
- 冒烟测试补充卡池数量、齐射指令固定10秒、描述同步、无敌人开火、炮塔转向和本地化消息断言。

验证：
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。

## ver0.45.2 - 速射塔与冰塔数值微调

- 速射塔等级3不再提高单枚弹丸伤害，保留三连发定位，避免等级3增量 DPS 过高。
- 速射塔等级3射击间隔倍率从 `0.70` 小幅下调到 `0.66`，用更密的弹幕感补偿移除伤害成长后的手感。
- 冰塔减速持续时间从 `2.2` 秒延长到 `3.0` 秒，提高控制分支相对火塔的功能价值。

验证：
- 本次为数值微调，未启动 Godot；已按脚本公式复核修改后的 DPS。

## ver0.45.3 - 增幅塔升级与全屏卡牌范围修复

- 修复增幅塔与临时卡牌增益共用倍率变量的问题：临时伤害/射程增益结束时不再清掉永久增幅效果。
- 增幅塔叠加后会作为独立升级目标显示在炮塔面板中，升级按钮显示“升级增幅”，并优先升级增幅层。
- 增幅塔升级费用提高为 `320` / `560` 金币，降低高等级炮塔叠满增幅后的数值爆炸风险。
- 全屏火焰、冻结保持真正全屏生效；稀有度不再改变范围，只强化伤害、减速/灼烧等效果数值。
- 敌人刷新间隔继续放慢，同时第十关额外增加敌群数量，强化终局压力。
- 敌人基础血量下调，前期第一波不再过肉；同时新增随关卡递增的血量倍率，让后续关卡尤其后期关卡保持压迫感。
- 冒烟测试补充增幅塔独立升级、永久倍率不被临时增益清除、全屏卡范围不随稀有度变化、第十关额外怪量等断言。
- 冒烟测试补充敌人基础血量降低、后续关卡血量倍率升高、敌人属性缓存按关卡区分等断言。
...
- 新增右上角游戏速度按钮，可在 `1x`、`1.5x`、`2x`、`4x` 间循环切换。
- 加速使用 Godot 全局时间缩放，覆盖刷怪、敌人移动、炮塔射击、卡牌倒计时与战斗特效等实时流程。
- 暂停仍使用场景树暂停，和游戏速度切换互不冲突；进入主场景时会恢复默认 `1x`，避免残留上一次测试或运行的倍速。
- 冒烟测试补充速度按钮创建、文本显示、共享点击音效绑定，以及四档速度循环断言。
...
- 修复炮塔套上增幅层后，原塔升级入口被增幅升级占用的问题；原塔升级和增幅升级现在拆成两个独立按钮。
- 增幅塔面板补充显示原塔等级、有效伤害/射程/间隔，以及增幅层等级、伤害加成和射程加成，避免信息不全。
- 原塔升级费用不再被增幅升级费用覆盖，增幅层仍保留较高升级费用，防止倍率叠加过快膨胀。
- 冒烟测试补充套增幅后双按钮显示、原塔升级不消耗增幅等级、增幅升级不消耗原塔等级等回归断言。

验证：
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。

## ver0.45.6 - 诱饵敌人投放节奏调整

- 嘲讽/诱饵敌人的血量倍率从 `3.4` 提高到 `4.5`，定位改为少量高血量的吸火力单位。
- 嘲讽/诱饵敌人不再按固定高频序号混入敌群，而是在可出现波次中均匀投放，单波控制在 `5` 到 `6` 个。
- 冒烟测试补充诱饵敌人血量倍率、同波基础敌人血量对比，以及单波低数量投放断言。

验证：
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。

## ver0.45.7 - 后期敌人血量倍率微调

- 第8关后新增独立血量压力段：从第9关开始，敌人关卡血量倍率额外提高，让后期敌群明显更肉。
- 保持前8关血量节奏不变，避免前中期被同步抬高难度。
- 冒烟测试补充第8关后血量倍率跃升和同波步兵血量变肉断言。

验证：
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。

## ver0.45.8 - 增幅塔升级面板独立化

- 将增幅塔升级区域从普通炮塔升级区域中拆出，独立显示在原塔升级窗口下方，避免按钮和信息互相挤占。
- 普通炮塔信息区只显示原塔等级、有效伤害、射程、射击间隔、原塔特性和原塔升级预览；增幅等级、加成和增幅升级预览移入独立增幅面板。
- 增幅升级按钮改为挂在独立增幅面板内，原塔升级/出售按钮不再因套增幅塔而被挪乱。
- 冒烟测试补充普通塔信息不混入增幅层、独立增幅面板显示、增幅升级按钮父节点和面板位置断言。
...
- 狙击塔基础开火间隔从 `1.55` 秒提高到 `1.85` 秒，让它更偏慢速高价值点杀定位。
- 狙击塔等级2升级的间隔倍率从 `0.94` 调整为 `0.98`，升级后仍保留重型射击节奏，避免中后期攻速过快。
- 狙击弹穿透后的伤害倍率从 `0.72` 下调到 `0.56`，首个目标仍吃满伤害，但后续穿透目标受到的伤害明显降低。
- 游戏内更新公告新增本次狙击塔数值调整条目，最新公告 id 同步为 `ver0.45.9`。
- 冒烟测试补充狙击塔基础开火间隔、等级2开火节奏和穿透衰减倍率断言。

验证：
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。

## ver0.45.10 - 支援层升级面板微调

- 套在炮塔上的支援层升级说明改用“加成”描述伤害与射程收益，不再在独立面板里重复堆叠“增幅”字样。
- 支援层升级按钮文案改为更短的“升级”，费用移到悬停提示里，满级状态显示为“满级”。
- 支援层升级按钮从原来的通栏按钮缩小为居中的小按钮，降低对原炮塔升级区域的视觉干扰。
- 冒烟测试补充支援层说明文案和按钮尺寸断言。

验证：
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。

## ver0.45.11 - 工程文档与升级界面布局调整

- 补齐软件工程交付文档：`TEST_PLAN.md`、`USER_MANUAL.md`、`COURSE_PROJECT_REPORT.md`。
- 展开手牌时，“收起手牌”按钮移动到折叠态“展开手牌”的同一屏幕位置，避开右侧炮塔升级/支援层区域。
- 支援层升级按钮移动到“等级x：xxxx”升级预览行右侧，按钮更贴近对应信息，不再单独占用面板下方一整行。
- 游戏内更新公告新增本次界面与文档更新条目，最新公告 id 同步为 `ver0.45.11`。
- 冒烟测试补充手牌按钮锚点和支援层升级按钮同行布局断言。
...
- 金色卡牌背景左右两侧各内切 `8px` 透明边缘，清除手牌堆叠时可见的相邻卡牌残影。
- 紫色卡牌背景左侧内切 `8px` 透明边缘，避免左边露出其他卡牌的亮边痕迹。
- 保持卡牌背景贴图画布仍为 `104x148`，不改变现有卡牌布局、堆叠位置和文字排版。
- 游戏内更新公告新增本次卡牌背景边缘清理条目，最新公告 id 同步为 `ver0.45.12`。

验证：
- 已实际运行 headless `CodeStyleAuditTest.gd`，结果通过（退出码0）。
- 已实际运行 headless `LogicSmokeTest.gd`，结果通过（退出码0）。

## ver0.45.13 - 软件工程文档 DOCX 导出

- 新增 `tools/export_docs_to_docx.py`，用于将 `docs` 目录下的 Markdown 文档批量导出为 Word `docx` 格式。
- 脚本不依赖 `python-docx` 等第三方库，直接使用 Python 标准库生成 WordprocessingML 包，便于在干净环境中复用。
- 已在 `docs/docx/` 下生成对应文档的 `docx` 版本，包括需求规格、设计说明、UML 图集、测试计划、用户手册和大作业报告等文档。
- 导出结果保留标题、段落、列表、简单表格和代码块文本，适合课程提交前在 Word/WPS 中继续排版微调。

验证：
- 已检查所有导出的 `docx` 包均包含 `[Content_Types].xml`、`_rels/.rels`、`word/document.xml` 和 `word/styles.xml`。
- 已解析所有导出的 `word/document.xml`，确认 XML 可读且包含正文段落/表格内容。

## ver0.45.14 - DOCX 文档中文命名

- 将 `docs/docx/` 下导出的 Word 文档统一改为中文文件名，便于课程提交和人工检查。
- 更新 `tools/export_docs_to_docx.py` 的输出映射，后续重新导出时会直接生成中文名，不会再回退为英文文件名。
- 保留 Markdown 源文档的英文文件名不变，继续满足仓库源码文件命名规范；中文命名仅用于提交用 `docx` 产物。

验证：
- 已确认 `docs/docx/` 下不存在英文名 `docx` 残留。
- 已检查中文名 `docx` 包结构，确认仍包含标准 Word 文件条目。

## ver0.45.15 - Image 版 UML 图生成

- 调用 Image 画图工具，根据 `docs/UML_DIAGRAMS.md` 中的 UML 图集生成 13 张 PNG 版 UML 图。
- 新增 `docs/uml_images/imagegen/`，保存用例图、组件图、核心类图、建塔/波次/攻击/卡牌顺序图、主流程活动图、三类状态图、数据模型图和部署图。
- 在 `docs/UML_DIAGRAMS.md` 开头补充 Image 版 UML 图的保存位置说明；Mermaid 源码继续作为精确结构版本保留。

验证：
- 已检查 `docs/uml_images/imagegen/` 下共 13 张 PNG 图片。
- 已确认所有图片尺寸均为 `1536x1024`。

## ver0.45.16 - Mermaid 精确版 UML 图片导出

- 将 `docs/UML_DIAGRAMS.md` 中的 13 个 Mermaid 代码块拆分为 `docs/uml_images/mermaid_src/` 下的 `.mmd` 源文件。
- 使用 Mermaid CLI 按源码精确渲染 UML 图片，新增 `docs/uml_images/mermaid_svg/` 与 `docs/uml_images/mermaid_png/` 两套导出结果。
- 新增 `tools/mermaid-puppeteer-config.json`，让 Mermaid CLI 复用本机 Chrome 渲染，避免额外下载 Chromium。
- 更新 `docs/UML_DIAGRAMS.md` 说明，明确 Mermaid 精确渲染版适合正式文档，Image 生成版仅适合视觉参考。

验证：
- 已成功渲染 13 张 SVG 与 13 张 PNG。
- 已确认所有导出文件均落盘到 `docs/uml_images/mermaid_svg/` 与 `docs/uml_images/mermaid_png/`。

## ver0.45.17 - 大作业报告扩写与 UML 图嵌入

- 扩充 `docs/COURSE_PROJECT_REPORT.md`，补充项目背景、需求分析、总体设计、详细设计、数值公式、测试质量保证、项目特色与总结展望等章节，使报告正文长度超过 4000 字符量级。
- 将 Mermaid 精确渲染版 UML 图片自动插入大作业报告正文，包括用例图、组件图、活动图、核心类图、顺序图、状态图、数据模型图和部署图等 13 张图。
- 根据当前代码补入炮塔基础数值、升级费用、支援层倍率、敌人波次数量/血量/刷新间隔公式、卡牌稀有度倍率和自动刷卡规则，避免报告数值与实现脱节。
- 新增 `tools/export_course_report_docx.ps1`，在无可用 Python 环境时直接用 PowerShell/OpenXML 从 Markdown 导出带图片的 `docx`。
- 更新 `docs/docx/大作业报告.docx`，正式 Word 版本已嵌入 13 张 Mermaid PNG UML 图片。

验证：
- 已统计 `docs/COURSE_PROJECT_REPORT.md`：175 行、9731 字符，满足 4000 字符以上扩写要求。
- 已运行 `tools/export_course_report_docx.ps1` 成功导出 `docs/docx/大作业报告.docx`，输出显示嵌入图片数量为 13。
- 已检查 `docx` 包结构，确认包含 `[Content_Types].xml`、`_rels/.rels`、`word/document.xml`、`word/_rels/document.xml.rels`、`word/styles.xml` 以及 13 个 `word/media/image*.png`。

## ver0.45.18 - 项目文件清理

- 删除历史 `.backup/` 目录，改为以 Git 历史和分支作为主要回退方式，避免本地快照持续膨胀。
- 删除 Godot 本地缓存 `.godot/`、旧压缩产物 `dist/`、继续上下文 `NEXT_SESSION_CONTEXT.md` 和空模板目录，保留源码、正式资产、测试、文档与工具脚本。
- 删除 `.test_pic/` 下的临时提示词、生成中间产物和旧素材文件；顶层空目录当前被 Windows 进程占用，已确认内部文件数为 0，待占用释放后可直接删除。
- 删除 `docs/uml_images/imagegen/` 视觉参考版 UML 图片，正式文档统一使用 Mermaid 精确版 `mermaid_png/` 与可再生源文件 `mermaid_src/`。
- 更新 `docs/UML_DIAGRAMS.md`，移除已删除 Image 参考图目录说明，避免文档指向不存在的路径。

验证：
- 已确认 `.backup/`、`dist/`、`NEXT_SESSION_CONTEXT.md`、空模板目录和 `docs/uml_images/imagegen/` 不再存在；`.godot/` 属于本地运行缓存，后续已由 Godot 重新生成，不纳入 Git。
- 已检查 `docs/COURSE_PROJECT_REPORT.md` 中 13 个 UML 图片引用均指向存在的 Mermaid PNG 文件。
- 清理后项目根目录仅保留源码、资产、场景、测试、文档、工具、Git 元数据和本地 Godot 测试可执行文件。

## ver0.45.20 - 代码答辩说明文档

- 新增 `docs/CODE_DEFENSE_GUIDE.md`，系统梳理项目代码架构、模块职责、运行原理、关键数值公式、代码规范和健壮性设计。
- 导出 `docs/docx/代码答辩说明文档.docx`，用于课程答辩时直接查看或打印。
- 文档补充答辩问答和展示顺序，便于围绕 `Main.gd`、`BuildManager.gd`、`WaveManager.gd`、`Tower.gd`、`Enemy.gd`、`Projectile.gd`、`GameUI.gd` 进行讲解。

## ver0.45.19 - 修复清理后的 Godot 启动缓存

- 重新启动 Godot 编辑器扫描工程并完整重建 `.godot/` 导入缓存，修复清理后首次打开/运行可能因为资源尚未重新导入而异常的问题。
- 明确 `.godot/` 是本地运行缓存：不应入库，但也不应在清理后不重建就交付给正在本机运行的项目。
- 保持此前删除的历史备份、旧压缩包、临时素材、空模板目录和 Image 参考版 UML 图不恢复，避免无用文件重新膨胀。

验证：
- 已运行 `Godot_v4.7-stable_win64_console.exe --headless --path . --editor --quit --verbose`，Godot 完成 170 项资源重新导入并正常退出。
- 已运行 `tests/LogicSmokeTest.gd`，逻辑冒烟测试退出码为 0。
- 已运行非 headless `Godot_v4.7-stable_win64_console.exe --path . --quit-after 3`，窗口模式启动正常并自动退出，退出码为 0。
